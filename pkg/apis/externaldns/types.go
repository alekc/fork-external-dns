...
func bindFlags(b flags.FlagBinder, cfg *Config) {
	...
	b.BoolVar("force-default-targets", "Force the application of --default-targets, overriding any targets provided by the source, except an explicit per-resource target annotation (external-dns.alpha.kubernetes.io/target), which still takes priority (DEPRECATED: This reverts to (improved) legacy behavior which allows empty CRD targets for migration to new state)", defaultConfig.ForceDefaultTargets, &cfg.ForceDefaultTargets)
